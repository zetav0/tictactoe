...
"Game Blast": three games — Tic Tac Toe and Connect Four ("4 en Raya"), each playable as local PvP, vs AI (easy/medium/hard), or online multiplayer over Firebase Realtime Database using 6-character room codes; plus Ball in the Hole ("Bola al Hoyo"), a single-player tilt-the-maze game (no PvP/AI/online — the home screen swaps the mode buttons for an info card when it's selected). Users have a profile (username + a preset multiavatar; custom photo upload was deliberately removed to avoid user-generated content) persisted with shared_preferences; app entry (`lib/app.dart`) routes to `ProfileSetupScreen` until a profile exists, then to `MainShell` — an IndexedStack with the three bottom-nav tabs (Play/History/Profile). Game screens are pushed on top of the shell and deliberately have no bottom nav.
...
**Ball in the Hole is the exception**: it is real-time physics, not turn-based, so it lives outside `BaseGameController`. `BallInHoleGame` (`lib/game/ball_in_hole_game.dart`, Flame) owns the physics loop — gravity comes from the accelerometer (sensors_plus; portrait-only so no orientation remapping) with an on-screen drag joystick as fallback, forwarded by `BallInHoleScreen` via `dragStart/dragUpdate/dragEnd`. Levels are ASCII grids in `lib/models/ball_level.dart` (`#` wall, `S` start, `G` goal, `X` trap) — `test/ball_level_test.dart` BFS-verifies every level has a trap-free path, so run it after editing levels. Progression (unlocked levels + best times) is `BallProgressService`, same ChangeNotifier-singleton pattern as `HistoryService`.
...
- **Sound:** `SoundService` (audioplayers) with the assets in `assets/sounds/` — move/win `.wav` SFX plus a looping background-music `.mp3`, each on its own player so they don't cut each other off. Music starts from `SoundService.init()` in `main.dart`; browsers block audio until a user gesture, so `app.dart` retries `ensureMusicStarted()` on the first pointer-down. It pauses with the app lifecycle and has a persisted on/off toggle in the Profile tab (`musicEnabled` ValueNotifier).
- **Match history:** every finished game is recorded from the controllers into `HistoryService` (ChangeNotifier singleton over shared_preferences, newest first, capped at 50) as a `MatchRecord`; `HistoryScreen` renders it live. `MatchRecord.result` is from the local player's perspective (player 1 locally, own role online). Ball in the Hole records completions only (`result: win` + the optional `level` field).
